...
export default function TenantDetailPage() {
  ...
  return (
    <div className="max-w-5xl">
      <Link href="/dashboard/tenants" className="text-sm text-gold hover:underline">Back to tenants</Link>
      <div className="flex items-start justify-between gap-4 mt-4 mb-8">
        <div><p className="text-xs uppercase tracking-wide text-gold mb-2">{tenant.tenant_type || "Unclassified"} · {tenant.status}</p><h1 className="text-2xl font-semibold">{tenant.name}</h1><p className="text-sm text-text-secondary mt-1">{tenant.incident_count} incidents · {tenant.user_count} users</p></div>
