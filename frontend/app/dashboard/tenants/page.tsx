...
export default function TenantsPage() {
  ...
  async function enterWorkspace(tenant: Tenant) {
    ...
    try {
      ...
      window.location.href = `/dashboard?tenant_id=${encodeURIComponent(tenant.id)}`;
    } catch (error) {
                ...
                <div key={tenant.id} className="py-4 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-text-secondary mt-1">
                      <span className="text-gold">{tenant.tenant_type || "Unclassified"}</span> · {tenant.incident_count} incidents · {tenant.user_count} users
                    </p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <Link href={`/dashboard/tenants/${tenant.id}`} className="text-sm text-text-secondary hover:text-gold">
                      View
                    </Link>
